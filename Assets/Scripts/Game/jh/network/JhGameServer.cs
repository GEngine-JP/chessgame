...
namespace Assets.Scripts.Game.jh.network
{
    public class JhGameServer : YxGameServer
    {
        ...
        /// 微信分享内容后台配置
        /// </summary>
        public void WechatShareTempate(object data)
        {
            YxTools.ShareFriend(GameData.CreateRoomInfo.RoomId.ToString(), GameData.CreateRoomInfo.RuleInfo);
        }
        ...
        void ShowClock(ISFSObject data)
        {
            ...
            {
                int readyCd = data.GetInt("readyCd");
                if (readyCd > 0)
                {
                    EventObj.SendEvent("TableEvent", "StartTime", readyCd);    
                }
            }
