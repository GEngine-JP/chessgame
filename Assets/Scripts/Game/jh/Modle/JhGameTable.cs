        ...
        public void OnRecieve(EventData data)
        {
            switch (data.Name)
            {
                ...
                case "ShowResult":
                    ...
                    break;
                case "StartTime":
                    OnStartTime(data.Data);
                    break;

            }
        }

        protected void OnStartTime(object p)
        {
            if (IsReadyCd&&IsCreatRoom && CreateRoomInfo.CurRound > 0 && RStatus <= RoomStatus.CanStart)
            {
                EventObj.SendEvent("TableViewEvent", "ReadyCountDown", p);
            }
        }
