...
namespace Assets.Scripts.Game.ddz2.DDzGameListener.InfoPanel
{
    ...
    public class PlayerRightListener : PlayerOtherListener
    {
        ...
        private void OnTypeOutCard(object sender, DdzbaseEventArgs args)
        {
            ...
            else if (curSeat == App.GetGameData<GlobalData>().GetRightPlayerSeat)
            {
                ShowSpeakSp.gameObject.SetActive(false);
        ...
        private void OnTypePass(object sender, DdzbaseEventArgs args)
        {
            if (args.IsfObjData.GetInt(RequestKey.KeySeat) == App.GetGameData<GlobalData>().GetSelfSeat)
                ShowSpeakSp.gameObject.SetActive(false);
            else if (args.IsfObjData.GetInt(RequestKey.KeySeat) == App.GetGameData<GlobalData>().GetRightPlayerSeat)
            {
        ...
        protected override void SetOtherPlayerData(ISFSObject servData)
        {
            ...
            var seat = App.GetGameData<GlobalData>().GetRightPlayerSeat;
        ...
        protected override void OnUserJoinRoom(object sender, DdzbaseEventArgs args)
        {
            ...
            if (!user.ContainsKey(RequestKey.KeySeat) || user.GetInt(RequestKey.KeySeat) != App.GetGameData<GlobalData>().GetRightPlayerSeat)
                return;
