...
{
    public class PlaybackComponent : BaseComponent
    {
        /// <summary>
        /// 每一帧播放间隔
        /// </summary>
        ...
        /// 回退播放 key < 0
        /// </summary>
        private Dictionary<int, Action<PlaybackFrameData>> mPlacbackHandlers = new Dictionary<int, Action<PlaybackFrameData>>();
        /// <summary>
        /// 持有服务器监听事件的对象缓存
        /// </summary>
        ...
        /// 上一帧消息
        /// </summary>
        protected PlaybackFrameData mLastFrameData;
        ...
        /// 执行回放
        /// </summary>
        protected IEnumerator<float> ExecutePlaybackFrame()
        {
            while (true)
            {
        ...
        {
            var types = GameCenter.Assets.TypeBinder.PlaybackLogicTypes();
            for (int i = 0; i < types.Count; i++)
            {
                ...
                if (response != null)
                {
                    ...
                    {
                        CollectionPlaybackHandler(response, methods[k]);
                    }
                }
            }
        }

        private void CollectionPlaybackHandler(IGameCommand response, MethodInfo method)
        {
            var handlerAtts = method.GetCustomAttributes(typeof(PlaybackHandlerAttrubute), false);
            if (null != handlerAtts && handlerAtts.Length > 0)
            {
                var handlerAtt = handlerAtts[0] as PlaybackHandlerAttrubute;
                var handler = CreateHandler<Action<PlaybackFrameData>>(response, method.Name);
                if (null != handler)
                {
                    var symbol = handlerAtt.Type == PlaybackType.Normal ? 1 : -1;
