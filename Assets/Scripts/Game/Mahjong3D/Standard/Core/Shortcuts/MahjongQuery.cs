...
namespace Assets.Scripts.Game.Mahjong3D.Standard
{
    public class MahjongQuery : ShortcutsPart, IGameEndCycle
    {
        ...
        public void ShowQueryTipOnOperate(IList<int> cards)
        {
            ...
            {
                GameCenter.Scene.MahjongGroups.PlayerHand.OnQueryMahjong(cards);
                GameCenter.EventHandle.Dispatch((int)UIEventProtocol.QueryBtnCtrl, new PanelTriggerArgs() { QueryBtnState = true });
            }
            ...
            {
                GameCenter.EventHandle.Dispatch((int)UIEventProtocol.QueryBtnCtrl, new PanelTriggerArgs() { QueryBtnState = false });
            }
        }
    }
}