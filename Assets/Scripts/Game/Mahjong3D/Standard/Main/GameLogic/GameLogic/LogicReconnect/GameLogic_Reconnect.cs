...
namespace Assets.Scripts.Game.Mahjong3D.Standard
{
    ...
    {
        [S2CResponseHandler(CustomClientProtocol.CustomTypeReconnectLogic)]
        ...
        private void UpdateMahjongGroup()
        {
            ...
            {
                int tempBao = DataCenter.Game.BaoCard;
                if (tempBao == 0)
                {
                    tempBao = 17;
                }
                //宝牌是否显示
                bool showBao = !base.DataCenter.ConfigData.AnBao && base.DataCenter.OneselfData.IsAuto;
        ...
        private void SetReconectCardState()
        {
            //设置打牌状态
            if (DataCenter.ReconectCardState == 0)
            {
                ...
                if (DataCenter.CurrOpChair == 0 && DataCenter.Players[0].IsAuto)
                {
                    ...
                    {
                        GameCenter.Network.C2S.ThrowoutCard(lastCard.Value);
                    }
                    else
                    {
        ...
        /// 显示麻将查询提示
        /// </summary>
        private void MahjongQueryHuTip()
        {
            if (!DataCenter.ConfigData.MahjongQuery && MahjongUtility.TingTipCtrl != 0) return;
            GameCenter.Shortcuts.MahjongQuery.Do(p => p.OnReconnectRecordMahjong());
        ...
        private void UpdateMahjongTable()
        {
            ...
            //设置上次出牌用户的箭头标记
            if (DataCenter.OldOpChair != MiscUtility.DefValue)
            {
