...
{
    public class MahPlayerHand_Qdjt : MahPlayerHand
    {
        ...
        {
            //MjToNormal();
            //EventDispatch.Dispatch((int)GameEventId.CancelTingIcon);
            //EventDispatch.Dispatch((int)UIEventId.LiangPaiTip, new EventData(0));
            ////CanChoose = cards;
            //bool flag = true;
            ...
            for (int i = 0; i < MahjongList.Count; i++)
            {
                ...
                //{
                //    MahjongList[i].Lock = true;
                //}
                //else
                //{
                //    MahjongList[i].SetThowOutCall(ChoosePai);
                //    MahjongList[i].ChangeWaitForXjfd(true);
            ...
            if (diaoCard > 0)
            {
                GameCenter.Network.C2S.Custom<C2SCustom>().Liangdao(new[] { diaoCard }, mChooseTingItem.Value);
                //    TingThrow = false;
                //    EventDispatch.Dispatch((int)NetEventId.OnSendLiangPai, new EventData(new[] { DiaoCard }, TingItem.Value));
                //    EventDispatch.Dispatch((int)UIEventId.HideChooseLiangPai);
                //    MjToNormal();
            }
            else
            {
                ...
                {
                    //        string LogCards = "";
                    //        foreach (int card in LiangCards)
                    //        {
                    //            LogCards += "[" + card + "]";
                    //        }
                    //        Debug.Log("发送亮牌值：" + LogCards);
                    //        TingThrow = false;
                    //        EventDispatch.Dispatch((int)NetEventId.OnSendLiangPai,
                    //            new EventData(LiangCards, TingItem.Value));
                    //        EventDispatch.Dispatch((int)UIEventId.HideChooseLiangPai);
                    //        MjToNormal();
                    GameCenter.Network.C2S.Custom<C2SCustom>().Liangdao(LiangCards, mChooseTingItem.Value);
                }
                else
                {
                    ...
                    //            item.Lock = true;
                    //        }
                    //        LiangCardsSave = LiangCards;
                    //        EventData evn = new EventData();
                    //        evn.data2 = GangCards;
                    //        EventDispatch.Dispatch((int)UIEventId.ShowChooseXJFD, evn);
        ...
        public void SendLiangPai(int[] ChooseGang)
        {
            ...
            //TingItem.Lock = false;
            //EventDispatch.Dispatch((int)NetEventId.OnSendLiangPai,
            //    new EventData(SendLiang.ToArray(), TingItem.Value));
            //EventDispatch.Dispatch((int)UIEventId.HideChooseLiangPai);
            //MjToNormal();

            GameCenter.Network.C2S.Custom<C2SCustom>().Liangdao(SendLiang, mChooseTingItem.Value);
        }
