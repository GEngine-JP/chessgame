...
namespace Assets.Scripts.Game.Mahjong3D.Standard
{
    ...
    public class PanelGameTriggers : UIPanelBase
    {
        ...
        {
            GameCenter.EventHandle.Subscriber((int)UIEventProtocol.ReadyBtnCtrl, ReadyBtnCtrl);
            GameCenter.EventHandle.Subscriber((int)UIEventProtocol.QueryBtnCtrl, QueryBtnCtrl);
            GameCenter.EventHandle.Subscriber((int)UIEventProtocol.QueryBtnCtrl, QueryBtnCtrl);
            GameCenter.EventHandle.Subscriber((int)GameEventProtocol.AiAgency, AgencyAi);
            SetLanguage();
        }
        ...
        public override void OnGetInfoUpdate()
        {
            ...
            if (db.Room.RoomType == MahRoomType.FanKa)
            {
                ...
                //邀请好友按钮控制
                if (db.ConfigData.InviteFriends && db.MaxPlayerCount != db.Players.CurrPlayerCount)
                {
        ...
        public void OnDismissRoom()
        {
            YxMessageBox.Show("是否要解散房间？", null, (box, btnName) =>
            {
                ...
                {
                    GameCenter.Network.C2S.OnDismissRoom((int)DismissFeedBack.ApplyFor);
                }
        ...
        {
            GameCenter.EventHandle.Dispatch((int)UIEventProtocol.ShowMahFriendsPanel);
        }
        ...
        {
            GameCenter.EventHandle.Dispatch((int)UIEventProtocol.ShowGameRule);
        }
        ...
        {
            GameCenter.EventHandle.Dispatch((int)UIEventProtocol.ShowGameExplain);
        }
        ...
        public void OnAiAgencyClick()
        {
            ...
            {
                GameCenter.EventHandle.Dispatch((int)GameEventProtocol.AiAgency, new AiAgencyArgs() { State = true });
                Group.SetActive(false);
            }
        }
