...
namespace Assets.Scripts.Game.jpmj
{
    public class JpTableData : TableData
    {
        ...
        /// 麻将的吃碰杠牌组
        /// </summary>
        [SerializeField] protected MahjongCpgGroup[] MahjongCpgGrps;

        [SerializeField] protected GameObject GameReadyBtn;
...
/*            if (cpgList != null)
            {
                int cdNum = 0;
                foreach (var gob in cpgList)
                {
                    var num = gob.transform.childCount;
                    cdNum += num > 3 ? 3 : num;
                }
