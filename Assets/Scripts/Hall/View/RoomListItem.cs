...
namespace Assets.Scripts.Hall.View
{
    ...
    public class RoomListItem : NguiListItem
    {
        ...
        protected override void OnChangeData(IItemData itemData)
        {
            ...
            YxTools.TrySetComponentValue(AnteLabel, YxUtiles.GetShowNumber(ante).ToString());
            long maxCoin;
            long.TryParse(_model.MaxGold, out maxCoin);
            YxTools.TrySetComponentValue(MaxCoinLabel, maxCoin < 1 ? "∞":YxUtiles.GetShowNumber(maxCoin).ToString(CultureInfo.InvariantCulture));
            YxTools.TrySetComponentValue(MinCoinLabel, YxUtiles.GetShowNumber(minCoin).ToString(CultureInfo.InvariantCulture));
        ...
        { 
            ...
            var showMinGold = YxUtiles.GetShowNumber(long.Parse(room.MinGold));
            var showMaxGold = YxUtiles.GetShowNumber(long.Parse(room.MaxGold));
            ...
            if (showMinGold> userCoin)//判断金币是否达到上限
            {
                YxMessageBox.Show(string.Format("非常抱歉!!!当前房间最低下限需要{0}金，您的金币只有{1}金，小于房间的最低下限不能进入。", showMinGold, userCoin));
                return;
            }
            if (showMaxGold > 0 && showMaxGold < (userCoin + userBankCoin))
            {
                YxMessageBox.Show(string.Format("非常抱歉!!!当前房间最高上限需要{0}金，您的金币已有{1}金(包含银行金币{2})，大于房间的最高上限将不能进入。", showMaxGold, userCoin, userBankCoin));
                return;
            }
