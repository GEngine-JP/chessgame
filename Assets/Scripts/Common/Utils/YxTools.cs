...
namespace Assets.Scripts.Common.Utils
{
    public static class YxTools
    {
        ...
        /// <param name="value">接收源</param>
        /// <param name="key">解析Key</param>
        public static void TryGetValueWitheKey(this Dictionary<string, object> dic, out Dictionary<string, object> value, string key)
        {
            if (dic.ContainsKey(key))
            {
                var data = dic[key] ?? new Dictionary<string, object>();
        ...
        /// <param name="value">接收源</param>
        /// <param name="key">解析Key</param>
        public static bool TryGetValueWitheKey(this Dictionary<string, object> dic, out List<object> value, string key)
        {
            var getState=false;
            if (dic.ContainsKey(key))
            {
                var changeData = dic[key] as List<object>;
                if (changeData  == null)
                {
